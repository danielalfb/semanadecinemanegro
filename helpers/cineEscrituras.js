export const CINEESCRITURAS = [
  {
    title: "Oceânica inquietude de ser",
    id: "1",
    movies: [
      {
        title: "SONHO EM RUÍNAS",
        titleEng: "SONHO EM RUÍNAS",
        subtitle: "PERNAMBUCO, 2024, 4'",
        description: `Um sonho traz à tona a história ainda não contada de uma bisavó negra.`,
        descriptionEng: ``,
        credits: `
            direção: Priscila Nascimento;
            roteiro: Priscila Nascimennto;
            produção: Priscila Nascimento;
            fotografia: Priscila Nascimento;
            montagem: Priscila Nascimento;
            arte e figurino: Priscila Nascimento;
            som: Priscila Nascimento;
            trilha sonora original: Priscila Nascimento;
            elenco principal:  Iyadirê Zidanes, Maria Rita do Nascimento;
            contato: waymoprinps@gmail.com
            `,
        image: "/images/movies/SONHO_EM_RUINAS.png",
      },
      {
        title: "MINHA PELE PRETA EM TERRA VERDE",
        titleEng: "MINHA PELE PRETA EM TERRA VERDE",
        subtitle: "ACRE, 2025, 21'",
        description: `No centro de uma Amazônia reconhecidamente indígena, Pio busca a própria identidade negra a partir de memórias e diálogos com seu amigo Cecílio, um jornalista ribeirinho engajado e empenhado em contar histórias do povo negro amazônico.`,
         credits: `
            direção: Teddy Falcão;
            roteiro: Teddy Falcão;
            produção: Lys Guimarães;
            fotografia: Teddy Falcão;
            montagem: Lucas Sá;
            arte e figurino: Hannah Lydia;
            som: Lucas Sá;
            trilha sonora original: Lucas Sá;
            elenco principal: Marcos Luanderson, Douglas Lamero;
            empresa produtora: Saturno Audiovisual;
            contato: maahac@gmail.com
            `,
        image: "/images/movies/MINHA_PELE_PRETA_EM_TERRA_VERDE.jpg",
      },
       {
        title: "ESTRANGEIRO",
        titleEng: "ESTRANGEIRO",
        subtitle: "CEARÁ, 2025, 16'",
        description: `Kaio não é deste planeta. Em uma noite de sábado, ele sai para se divertir com seus amigos terráqueos, mas acaba precisando lidar com a sensação de não pertencimento. Com a cultura reggae do Ceará como plano de fundo, Estrangeiro aborda as vivências de pessoas LGBTQIAPN+ periféricas e  faz uma celebração das diversidades racial, sexual e de gênero dentro da periferia`,
         credits: `
            direção: Edigar Martins;
            roteiro: Edigar Martins;
            produção: Angélica Rodrigues, Carmo Nobre;
            fotografia: Edigar Martins;
            montagem: Edigar Martins;
            arte: Amon Aristides, Edigar Martins;
            figurino: Amon Aristides;
            som: Inka D. Ibañez, Mike Dutra;
            trilha sonora original: Edigar Martins;
            elenco principal: Edigar Martins, Elber Holanda, Mayra Gabrielle, Pamella Rocha;
            empresa produtora: Carbono Criativo, Espaçonaves;
            contato: espaconaves7@gmail.com
            `,
        image: "/images/movies/ESTRANGEIRO.png",
      },
       {
        title: "NOTAS SOBRE A IDENTIDADE",
        titleEng: "NOTAS SOBRE A IDENTIDADE",
        subtitle: "DISTRITO FEDERAL, 2024, 20'",
        description: `Maria, Lia e Raquel vivem juntas. Três mulheres de mundos diferentes: uma cientista, uma professora e uma androide que, juntas, precisam preparar o São João da escolinha de Lia.`,
        credits: `
            direção: Marisa Arraes;
            roteiro: Marisa Arraes;
            produção: Marisa Arraes, Diego Sales de Castro;
            fotografia: Ana Luíza Meneses;
            montagem: Henrique Laterza;
            arte: Rosa Morbach, Sarah Noda;
            figurino:  Júlia Kali;
            som: Luiza Chagas, Martha Suzana;
            trilha sonora original: Ypu;
            elenco principal: Tuanny Araujo, Pâmela Germano, Lilia Tricarico;
            empresa produtora: Atlantique;
            contato: atlantiquefilmes@gmail.com
            `,
        image: "/images/movies/NOTAS_SOBRE_A_IDENTIDADE.jpg",
      },
    ],
  },
  {
    title: "Conversas para quando o tempo não existir",
    id: "2",
    movies: [
      {
        title: "MEU PAI E A PRAIA",
        titleEng: "MEU PAI E A PRAIA",
        subtitle: "BAHIA, 2024, 15'",
        description: `Em uma versão futurista de Salvador, Kinho fica desapontado com o pai quando ele diz que não pode levá-lo à praia. No dia seguinte, porém, o menino é surpreendido e, embora estranhe o comportamento do pai, decide aproveitar aquele tempo precioso ao seu lado.`,
        descriptionEng: ``,
        credits: `
            direção: Marcos Alexandre;
            roteiro: Marcos Alexandre;
            produção: Gabriela Correia;
            fotografia: Mamba Negra;
            montagem: Iris de Oliveira;
            arte e figurino: Alice Braz;
            som: Herison Pedro;
            trilha sonora original: Piratas F&M, Carlos do Complexo;
            elenco principal: Kaio Ribeiro, Heraldo de Deus, Aline Nepomuceno;
            empresa produtora: Gran Maître Filmes;
            contato: granmaitrefilmes@gmail.com
            `,
        image: "/images/movies/MEU_PAI_E_A_PRAIA.png",
      },
      {
        title: "A INVENÇÃO DO ORUM",
        titleEng: "A INVENÇÃO DO ORUM",
        subtitle: "ESPÍRITO SANTO, 2025, 18'",
        description: `Ao retornar para sua cidade natal, a drag queen Jéssica revive, pelas memórias da própria mãe, “uma história para não se perder”: o dia em que ela fugiu de Novo Céu para ser feliz. Agora, entrelaçada numa relação atemporal de saudade, culpa e redenção, Jéssica volta para entender melhor suas escolhas mais profundas.`,
        credits: `
            direção: Paulo Sena;
            roteiro: Paulo Sena;
            produção: Juliana Sena;
            fotografia: Lourenço Diniz;
            montagem: Lourenço Diniz;
            arte: Emerson Evêncio;
            figurino: Chica Chiclete;
            som: Marcos Vianna;
            trilha sonora original: Wanderson Lopes;
            elenco principal: Suely Bispo, Bené Freire, Jéssica Telles (Drag Queen);
            empresa produtora: CABELOSECO Soluções Criativas;
            contato: paulosssena@hotmail.com
            `,
        image: "/images/movies/A_INVENCAO_DO_ORUM.jpg",
      },
       {
        title: "AMOR É UM RIO DE ÁGUAS ESCURAS",
        titleEng: "AMOR É UM RIO DE ÁGUAS ESCURAS",
        subtitle: "MINAS GERAIS, 2025, 16'",
        description: `O rio de afetos negros é profundo, múltiplo e capaz de acolher uma comunidade inteira. A partir das questões que confluem no sentir do cotidiano de dois jovens negros, Jazz e Omar,  Amor é um rio de águas escuras traça um caminho percorrido por pessoas negras de diferentes lugares e com diversas idades e crenças em busca de vivenciar o amor. Um mergulho profundo em águas escuras, que, entre o mistério e o risco, ainda assim é capaz de nos provar que estamos vivos!`,
       credits: `
            direção: Aziza Eduarda;
            roteiro: Aziza Eduarda;
            produção: Amanda Maciel, Clarisse Dias, Pedro Henrique dos Santos;
            fotografia: Aziza Eduarda, Ysabella Lyra;
            montagem: Aziza Eduarda;
            arte:  Raique Nic, Ysabella Lira;
            figurino: Larioh, Roberta Curcio;
            som: Raul Dias, RaH BXD;
            trilha sonora original: Imane Rane, Tinguê;
            elenco principal: Cicero Cristo, Thales Barbosa, Amanda Oliveira, Babalorixá Ricardo de Oyá, Francisco Oliveira, Bruna Paula Martins, Oruam de Paula Martins, Carolina Oliveira, Italo Procopio, Iúna Ferreira, Irmandade os Ciriacos, Larioh,  Linde O Artista, Miss Black, Nico Elay, Vanderleia Reis, Rhaiane do Rosário, Megan Gabriele, Clarisse Ribeiro, Clarelis Ribeiro, Vanessa Profeta;
            empresa produtora: Ananse Estudios;
            contato: azizaeduardaxavier@gmail.com, ananseprodutora@gmail.com
            `,
        image: "/images/movies/AMOR_E_UM_RIO_DE_AGUAS_ESCURAS.jpeg",
      },
       {
        title: "ESPELHO DA MEMÓRIA",
        titleEng: "ESPELHO DA MEMÓRIA",
        subtitle: "SÃO PAULO, 2024, 16'",
        description: `Ao reunir arquivos de infância, gravações recentes e referências audiovisuais, Guinho  compartilha momentos da própria história e da relação com sua avó, Cida, que deu início às filmagens caseiras da família. Espelho da Memória é uma reflexão sobre o poder da memória, da arte e do afeto.`,
       credits: `
            direção: Filipe Travanca, Roberto Simão;
            roteiro: Filipe Travanca, Roberto Simão;
            produção: Giovanna Rotter;
            fotografia: Otavio Vidal ;
            montagem: Vinícius Cerqueira;
            arte e figurino: Ella Gebara;
            som: Laís Vallina;
            trilha sonora original: Angie Lopez;
            elenco principal: Rita Niza, Fellipe Marcelino, Guilherme Nunes;
            empresa produtora: Praiana Filmes;
            contato: giovannarotter@gmail.com
            `,
        image: "/images/movies/ESPELHO_DA_MEMORIA.jpg",
      },
    ],
  },
  {
    title: "De Atenas a Jamaica",
    id: "3",
    movies: [
      {
        title: "GINGA REGGAE",
        titleEng: "GINGA REGGAE",
        subtitle: "MARANHÃO, 2024, 51'",
        description: `Um documentário musical sobre a cultura reggae na capital do Maranhão, conhecida como a Jamaica Brasileira. Na ilha de São Luís, a história do reggae se confunde com a biografia de Célia Sampaio, cantora pioneira do gênero no Brasil. Considerada a “Dama do Reggae”, sua perspectiva negra feminina desenha a estética reggae maranhense, a partir da qual a dança, as radiolas, os discotecários e o dançar agarradinho criaram, há mais de 40 anos, conexões entre territórios de resistência negra na diáspora.`,
        descriptionEng: ``,
        credits: `
            direção: Naýra Albuquerque;
            roteiro: Naýra Albuquerque;
            produção: Thainá Nogueira, Luiza Fernandes, Naýra Albuquerque;
            fotografia: Luiza Fernandes  ;
            montagem: Naýra Albuquerque;
            arte: Gê Viana;
            figurino: Afefé Criações;
            som: Juan Gusmão;
            elenco principal: Célia Sampaio, Magno Roots, Dj Natty Nayfson, Alberto Novo Quilombo e Professor Carlos Benedito, Jordman Emmanuel Silva (Garotinho Beleza), Gleiciane Morais, Célia Sampaio, Frank Rondinelly, Allana Martins;
            empresa produtora: Zawa Filmes;
            contato: nayra.albuquerque@gmail.com
            `,
        image: "/images/movies/GINGA_REGGAE.jpeg",
      },
    ]
  },
  {
    title: "Manifestos por uma história da memória negra",
    id: "4",
    movies: [
      {
        title: "MAR DE DENTRO",
        titleEng: "MAR DE DENTRO",
        subtitle: "PERNAMBUCO, 2024, 8'",
        description: `Um retrato da incansável insubmissão de Preto Sérgio e sua busca pela revelação da história não contada, desamarrando os nós a partir dos bons ventos que o levam ao mar de fora e ao mar de dentro.`,
        descriptionEng: ``,
        credits: `
            direção: Lia Letícia;
            roteiro: Lia Letícia;
            produção: L'Oro Produções, Daniela Azevedo;
            fotografia: Adalberto Oliveira;
            montagem: Letícia Barros;
            som: Priscila Nascimento;
            trilha sonora original: Edgar;
            empresa produtora: Dona Ledy Produções Artísticas, Vilarejo Filmes;
            contato: irene@urbangroup.biz
            `,
        image: "/images/movies/MAR_DE_DENTRO.jpg",
      },
      {
        title: "VOZ ZOV VZO",
        titleEng: "VOZ ZOV VZO",
        subtitle: "RIO DE JANEIRO, 2025, 51'",
        description: `Numa noite de quarta-feira, no ano de 1975, Jade aquece a voz em um estúdio de som. Não esperava, porém, ser interrompida pela chegada inesperada de uma velha amiga, que, por meio de uma carta, traz de volta a irmã de Jade. Os silêncios que acontecem em seguida ecoam pelo espaço, e Jade se encontra com eles, e com eles conversa. Voz Zov Vzo é uma alegoria musical acerca dos silenciamentos na ditadura militar e das vozes que deixam de ser ouvidas.`,
        credits: `
            direção: Yhuri Cruz;
            roteiro: Yhuri Cruz;
            produção: Yhuri Cruz, Pê Moreira;
            fotografia: Clara Cavour;
            montagem: Edson Lemos edt, Luisa Darks;
            arte: Alex Reis e Yhuri Cruz;
            figurino: Julliana Araújo ;
            som: Estevão Casé;
            trilha sonora original: C-Afro Brasil, Yhuri Cruz;
            elenco principal: Jade Maria Zimbra, Caju Bezerra, Almeida da Silva, Pedro Bento, Alex Reis, Dani Câmara;
            empresa produtora: Por Extenso Produções;
            contato: porextensoproducoes@gmail.com
            `,
        image: "/images/movies/VOZ_ZOV_VZO.jpg",
      },
    ],
  },
  {
    title: "Para todas as cartas de amor",
    id: "5",
    movies: [
      {
        title: "TIGREZZA",
        titleEng: "TIGREZZA",
        subtitle: "BAHIA, 2025, 35'",
        description: `Ao mesmo tempo em que tenta fazer Darnley se reconectar consigo mesmo e ser mais livre, Tigrezza procura um boy em aplicativos de relacionamento, disposta a negociar a própria liberdade.`,
        descriptionEng: ``,
        credits: `
          direção: Vinícius Eliziário;
          roteiro: Vinícius Eliziário;
          produção: Icaro Piton;
          fotografia: Wendel Assis;
          montagem: Rafael Fernando Oliveira, Vinícius Eliziário;
          arte: Vinicius Consi;
          figurino: Lattyna;
          som: Marise Urbano, Gabriel Muniz;
          trilha sonora original: MAPA;
          elenco principal: Paulilo, Vittor Adel;
          empresa produtora: Boca de Filmes;
          contato: contato.bocadefilmes@gmail.com;
        `,
        image: "/images/movies/TIGREZZA.jpg",
      },
      {
        title: "O CADERNO DE AVENCA",
        titleEng: "O CADERNO DE AVENCA",
        subtitle: "MINAS GERAIS, 2024, 20'",
        description: `Avenca é uma jovem atriz que vem enfrentando dificuldades nos trabalhos com o teatro. Em meio às adversidades, sua mãe lhe entrega um caderno verde que estava perdido em um guarda-roupa, entre coisas antigas da filha, inspirando Avenca a dar um novo rumo para a própria vida e ir atrás do sonho de trabalhar com cinema.`,
        credits: `            
          direção: Aisha Brunno;
          roteiro: Aisha Brunno, Bramma Bremmer;
          produção: Aisha Brunno, Bramma Bremmer, Simone Abreu;
          fotografia: Wend Fernandes;
          montagem: Aisha Brunno, Bramma Bremmer, Higor Gomes;
          arte: Breno Oliveira;
          figurino: Will Soares;
          som: Fabricio Shuch;
          trilha sonora original: Black Josie;
          elenco principal: Aisha Brunno, Gui Ventura, Anair Patrícia (Naná), Laura Matias Pinheiro, Carlandréia Ribeiro;
          empresa produtora: Anõmala Estudia;
          contato: ocadernodeavenca@gmail.com
         `,
        image: "/images/movies/O_CADERNO_DE_AVENCA.png",
      },
       {
        title: "BÁRBARA",
        titleEng: "BÁRBARA",
        subtitle: "PARÁ, 2024, 18'",
        description: `Bárbara vive o fim do luto da morte do marido quando decide que, depois de ter passado tantos anos priorizando a maternidade e servindo ao papel de esposa, é hora de iniciar um novo ciclo. Assim, aos 60 anos e em busca de descobrir sua própria identidade como mulher, ela entra no mundo dos aplicativos de namoro e se vê diante de um campo de novas possibilidades e desafios inesperados.`,
        credits: `
          direção: Joyce Cursino;
          roteiro: Flávia Ribeiro;
          produção: Joyce Cursino;
          fotografia: Saturação;
          montagem: Saturação;
          arte: Bia Sena;
          figurino: Nanan Falcão;
          som: Renata Beckman;
          trilha sonora original: Angela Maria - Cinderela. Dona Onete - Poder de Sedução. Zé Diogo - Me chama de preto.;
          elenco principal: Natal Silva, Edvaldo Azul, Lúcia Corrêa, Pelé do Manifesto, Zé Diogo, Gabriel Becker, Charles Santana, Isma Rodrigues, Loreta Carvalho, Tio Black, Edenilza Borges, Ana Carolina Tourão, Helena Ressoal;
          empresa produtora: Negritar Filmes e Produções;
          contato: negritarcomunicacao@gmail.com;
        `,
        image: "/images/movies/BARBARA.png",
      },
    ],
  },
  {
    title: "Nosso caminho não é uma linha reta",
    id: "6",
    movies: [
      {
        title: "VBP (VACAS BRANCAS PREGUIÇOSAS)",
        titleEng: "",
        subtitle: "SÃO PAULO, 2024, 19'",
        description: `Após chamar uma colega de classe de “vaca branca preguiçosa”, uma jovem estudante negra entra em uma saga virtual para se livrar do cancelamento.`,
        credits: `
          direção: Asaph Luccas;
          roteiro: Asaph Luccas;
          produção: Carol Santos, Leo Domingos;
          fotografia: Tatiane Ursulino;
          montagem: Guilherme Candido;
          arte: Gabriel Soares;
          figurino: Oliv Barros;
          som: Ayo Lima;
          trilha sonora original: Enzo dicarlo;
          elenco principal: Mavi Lucena, Gabriela Lang, Gabrelú, Inara dos Santos;
          empresa produtora: Coletivo Gleba do Pêssego;
          contato: coletivoglebadopessego@gmail.com
        `,
        image: "/images/movies/VBP.jpg",
      },
      {
        title: "QUEIMANDO POR DENTRO",
        titleEng: "QUEIMANDO POR DENTRO",
        subtitle: "PERNAMBUCO, 2024, 16'",
        description: `Tendo nascido em uma família evangélica neopentecostal, Samuel cresceu no coração de uma religião que ganhou grande influência no Brasil com o passar dos anos. Com isso, porém, à medida que começa a explorar a própria sexualidade e abraçar sua identidade queer, o jovem tem o mundo abalado por seu pai quando, de forma abrupta, ele o proíbe de dançar na igreja. Um ponto de virada em sua vida se torna iminente.`,
        credits: `
          direção: Matheus Farias, Enock Carvalho;
          roteiro: Matheus Farias, Enock Carvalho;
          produção: Matheus Farias, Enock Carvalho;
          fotografia: Gustavo Pessoa;
          montagem: Matheus Farias;
          arte: Joana Claude;
          figurino: Libra Lima;
          som: Edson Secco;
          elenco principal: Pedro Lucas;
          empresa produtora: Gatopardo Filmes;
          contato: info@gatopardofilmes.com
        `,
        image: "/images/movies/QUEIMANDO_POR_DENTRO.jpg",
      },
       {
        title: "MÃE DE OURO",
        titleEng: "MÃE DE OURO",
        subtitle: "MINAS GERAIS, 2024, 14'",
        description: `Numa noite sem lua, Tiana se vê em um beco sem saída, lutando para entender a morte de sua irmã.`,
        credits: `
          direção: Maick Hannder;
          roteiro: Maick Hannder;
          produção: Bruno Greco, Jacson Dias;
          fotografia: Fernanda de Sena;
          montagem: Maick Hannder;
          arte: Breno Henrique;
          figurino: Matheus Antunes;
          som: Ramon Modenesi;
          elenco principal: Carlandréia Ribeiro, Otávio Augusto de Moura Assis, Edu Costa, Clarice Prates, Flavi Lopes;
          empresa produtora: Ponta de Anzol Filmes;
          contato: producao@pontadeanzol.com.br
        `,
        image: "/images/movies/MAE_DE_OURO.jpg",
      },
       {
        title: "E SEU CORPO É BELO",
        titleEng: "E SEU CORPO É BELO",
        subtitle: "RIO DE JANEIRO, 2024, 23'",
        description: `Numa noite de soul, Carlos e Tony sangram suas mágoas… Na década de 1970, em meio às festas Black do subúrbio carioca, Carlos reencontra seu ex-namorado Tony acompanhado de outra pessoa, reacendendo mágoas embaladas pelo soul. E SEU CORPO É BELO é um misto de terror, romance e musical.`, 
        credits: `
          direção: Yuri Costa;
          roteiro: Yuri Costa;
          produção: Gabo M. Barros, Yuri Costa, Janyne Sousa, Luciana Druzina, Daniela Israel;
          fotografia: Max Chagas;
          montagem: Vinicius Silva;
          arte: Caroline Meirelles;
          figurino: Fernanda Cunha, Lydiane Carney;
          som: Kiko Ferraz e Ricardo Costa;
          elenco principal: João Pedro Oliveira, Paulo Guidelly, Dandara Lorena;
          empresa produtora: Barca Aberta Produções, Vértebra Artes, Druzina Content, Bactéria Filmes, KF Studios;
          contato: costayuriyc@gmail.com;
        `,
        image: "/images/movies/E_SEU_CORPO_E_BELO.png",
      },
    ],
  },
]
