  ...
  if (type === "primary") {
    return (
      <button
        onClick={onClick}
        type='button'
        className={`text-sm tablet:text-base p-1 laptop:p-2 m-1 laptop:m-2 rounded-lg transition-all duration-300 ease-out first:ml-0 hover:scale-105 cursor-pointer active:scale-100 link ${classes}`}
      >
        {children}
      </button>
    );
